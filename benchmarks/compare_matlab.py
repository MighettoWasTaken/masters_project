...
def _parse():
    ...
    p.add_argument(
        "--mat",
        default=os.path.join(_HERE, "TestDataSet_64", "1con0.mat"),
...
def print_comparison(ref, lib_rates, lib_power, lib_S, lib_f, lib_t, seed):
    ...
    for pop in POPS:
        ...
        print(
            f"  {pop:8s}  {rm:6.1f} ± {rs:4.1f}      {lm:6.1f} ± {ls:4.1f}    {delta:+7.1f}{flag}"
        )
    ...
    if common_mask.sum() > 1:
        ...
        if ref_S_common.std() > 0 and lib_S_common.std() > 0:
            ...
            print(
                f"  Spectral shape correlation     : {corr:+.3f}  (1.0 = identical shape)"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Running library model  (tmax={ref['tmax']:.0f}ms, n={ref['n']}, "
